...
function* parsePage(/** @type {DataView} */ pageDataView) {
  ...
  switch (pageType) {
    ...
    case 0x0A: {
      ...
      // console.log('Leaf index page has cells:', cellCount)

      for (let index = 0; index < cellCount; index++) {
        // varint reading Number of bytes of payload
        const keyVarint = new VarInt(new DataView(buffer, offset, 9))
        // console.log('Key varint', keyVarint.value)
        // Payload
        yield* yieldBlob('FFDAC1', keyVarint.byteLength, `Key varint (${keyVarint.value})`, new DataView(buffer, offset, keyVarint.byteLength))

        // Only offset by number of bytes used by varint
        offset += keyVarint.byteLength
  ...
  const expectedPageEndOffset = pageDataView.byteOffset + pageDataView.byteLength

  if (offset !== expectedPageEndOffset) {
    if (offset < expectedPageEndOffset) {
      // console.log(buf2hex(new DataView(buffer, offset, expectedPageEndOffset - offset).buffer.slice(0, 100)))
      throw new Error(
        `The offset after marking is too early: ${offset}. The expected end of the page should be at: ${expectedPageEndOffset}. (${pageDataView.byteLength} bytes after page start ${pageDataView.byteOffset})`
      )
    } else {
      throw new Error(
        `The offset after marking is too late: ${offset}. The expected end of the page should be at: ${expectedPageEndOffset}. (${pageDataView.byteLength} bytes after page start ${pageDataView.byteOffset})`
      )
    }
  }
}
